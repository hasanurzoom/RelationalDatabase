...
echo "$($PSQL "SELECT MAX(winner_goals) | MAX(opponent_goals) FROM games")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id=games.winner_id WHERE winner_id>0 ORDER BY name")"
